...
function library:CreateWindow()
    ...
    TopTitle.Text = player.Name .. " | Overhaul.gg"
    ...
    function containers:CreateContainer(name)
        ...
        local totalContainerSize = 0
            Container.ChildAdded:Connect(function(instance)
                task.wait(.5)
                if instance:IsA("Frame") then
                    totalContainerSize += instance.Size.Y.Offset + instance.Parent:WaitForChild("UIListLayout").Padding.Offset
                    Container.CanvasSize = UDim2.new(0,0, 0,0 + totalContainerSize + 30)
                end
            end)

            Container.ChildRemoved:Connect(function(instance)
                if instance:IsA("Frame") then
                    totalContainerSize -= instance.Size.Y.Offset
                    Container.CanvasSize = UDim2.new(0,0, 0,0 + totalContainerSize - 30)
                end
            end)
        ...
        function sections:CreateSection(name)
            ...
            function sectionExtra:CreateButton(name, callback)
                ...
                calback = callback or function() end
            ...
            function sectionExtra:CreateSlider(text, minvalue, maxvalue, callback)
                ...
                SliderButton.MouseButton1Down:Connect(function()
                    ...
                    SliderClip.Size = UDim2.new(0, math.clamp(mouse.X - SliderClip.AbsolutePosition.X, 0, 400), 0, 4)
            ...
            local totalSectionSize = 0
            Section.ChildAdded:Connect(function(instance)
                task.wait(.1)
                if instance:IsA("TextButton") or instance:IsA("TextLabel") then
                    totalSectionSize += instance.Size.Y.Offset + instance.Parent:WaitForChild("UIListLayout").Padding.Offset
                    Section.Size = UDim2.new(0.9,0, 0,0 + totalSectionSize + 6)
                end
            end)

            Section.ChildRemoved:Connect(function(instance)
                if instance:IsA("TextButton") or instance:IsA("TextLabel") then
                    totalSectionSize = totalSectionSize - instance.Size.Y.Offset
                    Section.Size = UDim2.new(0.9,0, 0,0 + totalSectionSize - 6)
                end
            end)
            return sectionExtra
